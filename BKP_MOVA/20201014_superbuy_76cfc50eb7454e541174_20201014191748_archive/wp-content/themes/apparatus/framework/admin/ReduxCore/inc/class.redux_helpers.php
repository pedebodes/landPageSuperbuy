    ...
    if ( ! class_exists( 'Redux_Helpers' ) ) {
        ...
        class Redux_Helpers {
            ...
            public static function isLocalHost() {
				global $wp;
                return ( isset( $wp->request) && ( $wp->request === '127.0.0.1' ||  $wp->request === 'localhost' )) ? 1 : 0;
            }
                    ...
                    foreach ( $files as $file ) {
                        ...
                        if ( $theme_file ) {
                            ...
                            if ( $core_version && ( empty( $theme_version ) || version_compare( $theme_version, $core_version, '<' ) ) ) {
                                ...
                                $found_files[ $plugin_name ][] = sprintf( esc_attr__( '<code>%s</code> version <strong style="color:red">%s</strong> is out of date. The core version is %s', 'apparatus' ), str_replace( WP_CONTENT_DIR . '/themes/', '', $theme_file ), $theme_version ? $theme_version : '-', $core_version );
                            } else {
