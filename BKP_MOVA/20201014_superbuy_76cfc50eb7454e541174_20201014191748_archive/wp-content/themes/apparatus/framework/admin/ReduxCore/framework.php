        ...
        class ReduxFramework {
            ...
            public static function init() {
                ...
                if ( self::$_is_plugin == true || self::$_as_plugin == true ) {
                    ...
                    if ( strpos( Redux_Helpers::cleanFilePath( __FILE__ ), Redux_Helpers::cleanFilePath( get_template_directory() ) ) !== false ) {
						...
						$check_full_url = trailingslashit( get_stylesheet_directory_uri() . $relative_url );
						if(preg_match("/{$search_certain_content}/i", $check_full_url)) {
                        self::$_url   = trailingslashit( $relative_url );
						} else {
						self::$_url   = $check_full_url;
            ...
            public function __construct( $sections = array(), $args = array(), $extra_tabs = array() ) {
                ...
                if ( empty ( $this->args['menu_title'] ) ) {
                    $this->args['menu_title'] = esc_html__( 'Options', 'apparatus' );
                }

                if ( empty ( $this->args['page_title'] ) ) {
                    $this->args['page_title'] = esc_html__( 'Options', 'apparatus' );
                }
            ...
            private function _internationalization() {
                 ...
                 * @param string     'apparatus'  text domain
                 */
                //                $locale = apply_filters( "redux/textdomain/{$this->args['opt_name']}", get_locale(), 'apparatus' );
                //
                ...
                $loaded = load_plugin_textdomain( 'apparatus', false, $basepath . 'languages');

                if ( !$loaded ){
                    $loaded = load_muplugin_textdomain( 'apparatus', $basepath . 'languages' );
                }

                if ( !$loaded ){
                    $loaded = load_theme_textdomain( 'apparatus', $basepath . 'languages' );
                }

                if ( ! $loaded ) {
                    $locale = apply_filters( 'plugin_locale', get_locale(), 'apparatus' );
                    $mofile = dirname( __FILE__ ) . '/languages/redux-framework-' . $locale . '.mo';
                    load_textdomain( 'apparatus', $mofile );
                }
            }
...
//                    }
                ...
                if ( ! empty ( $this->typography ) && ! empty ( $this->typography ) && filter_var( $this->args['output'], FILTER_VALIDATE_BOOLEAN ) ) {
                    ...
                    if ( $this->args['async_typography'] && ! empty ( $this->typography ) ) {
                        ...
                        <script>
                            ...
                            WebFontConfig['google'] = {families: [<?php echo esc_attr($typography->makeGoogleWebfontString ( $this->typography )) ?>]};

                            (function() {
                                var wf = document.createElement( 'script' );
                                wf.src = <?php ReduxFramework::$_url . 'assets/js/vendor/webfont' . $this->min . '.js' ?>;
                        ...
                        $protocol =  ! empty (is_ssl()) ? "https:" : "http:";
            ...
            public function _load_page() {
                ...
                if ( true == $this->show_hints ) {
                    ...
                    $dismiss = 'true';
                    $s       = esc_html__( 'Enable', 'apparatus' );

                    // Values for disabling hints.
                    if ( 'true' == $hint_status ) {
                        $dismiss = 'false';
                        $s       = esc_html__( 'Disable', 'apparatus' );
                    }

                    // Make URL
                    $url = '<a class="redux_hint_status" href="?dismiss=' . $dismiss . '&amp;id=hints&amp;page=' . $curPage . '&amp;tab=' . $curTab . '">' . $s . ' hints</a>';

                    $event = esc_attr__( 'moving the mouse over', 'apparatus' );
                    if ( 'click' == $this->args['hints']['tip_effect']['show']['event'] ) {
                        $event = esc_attr__( 'clicking', 'apparatus' );
                    }

                    // Construct message
                    $msg = sprintf( esc_attr__( 'Hints are tooltips that popup when %d the hint icon, offering addition information about the field in which they appear.  They can be %d d by using the link below.', 'apparatus' ), $event, strtolower( $s ) ) . '<br/><br/>' . $url;

                    // Construct hint tab
                    $tab = array(
                        'id'      => 'redux-hint-tab',
                        'title'   => esc_attr__( 'Hints', 'apparatus' ),
                        'content' => '<p>' . $msg . '</p>'
                    );
            ...
            private function get_default_output_string( $field ) {
                ...
                if ( ! empty ( $default_output ) ) {
                    $default_output = esc_attr__( 'Default', 'apparatus' ) . ": " . substr( $default_output, 0, - 2 );
                }
            ...
            public function ajax_save() {
                if ( ! wp_verify_nonce( $_REQUEST['nonce'], "redux_ajax_nonce" . $this->args['opt_name'] ) ) {
                    echo json_encode( array(
                        'status' => esc_attr__( 'Invalid security credential.  Please reload the page and try again.', 'apparatus' ),
                        'action' => ''
                    ) );
                ...
                if ( ! self::current_user_can( $this->args['page_permissions'] ) ) {
                    echo json_encode( array(
                        'status' => esc_attr__( 'Invalid user capability.  Please reload the page and try again.', 'apparatus' ),
                        'action' => ''
                    ) );
                ...
                if ( ! empty ( $_POST['data'] ) && ! empty ( $redux->args['opt_name'] ) ) {
                    ...
                    if ( $do_reload || ( isset ( $values['defaults'] ) && ! empty ( $values['defaults'] ) ) || ( isset ( $values['defaults-section'] ) && ! empty ( $values['defaults-section'] ) ) || ( isset ( $values['import_code'] ) && ! empty ($values['import_code']) ) || ( isset ( $values['import_link'] ) && ! empty ($values['import_link']) ) ) {
                        ...
                        echo json_encode( array( 'status' => esc_attr__( 'Your panel has no fields. Nothing to save.', 'apparatus' ) ) );
                    }
                }
                if ( isset ( $this->transients['run_compiler'] ) && $this->transients['run_compiler'] ) {
